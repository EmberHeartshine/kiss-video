...
kiss-video is intended to serve MP4 files using a standard video codec. You could `s/\.mp4/.webm/i` in `viewEp.js` to serve WEBM instead, but not every browser (*cough cough Apple*) can handle all media types as embedded video.
